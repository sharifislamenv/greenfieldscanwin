...
const ScanPage = () => {
  ...
  useEffect(() => {
    ...
    // --- UPGRADE: Console logging for review ---
    ...
    if (data) {
      ...
      if (parts.length === 7) { 
        ...
        // Data string for verification MUST match the exact string signed on the backend
        ...
        setStep('invalid-format'); // A new step to handle incorrect format
      }
    } else {
      console.log('ScanPage: No "d" parameter found in URL. Waiting for QR scan or manual input.'); //
      // Optionally, you might want to redirect to a help page or show a prompt here
    }
  }, [location]);
    ...
    <div className="scan-container">
      ...
      {step === 'location-verification' && (
        <div className="location-check">
          ...
          <p className="loading-message">Checking your proximity to the QR code...</p>
        </div>
      )}

      {step === 'location-mismatch' && (
        <div className="error-message">
          <h2>Location Mismatch!</h2>
          <p>You must be within 100 meters of the QR code to participate.</p>
      ...
      {step === 'location-error' && (
        <div className="error-message">
          ...
          <p>Could not get your location. Please ensure location services are enabled for your browser.</p>
          <button onClick={() => navigate('/')}>Return to Home</button>
        </div>
      )}

      {step === 'invalid-signature' && (
        <div className="error-message">
          <h2>Invalid QR Code!</h2>
          <p>The QR code data is invalid or has been tampered with. Please scan an official Greenfield QR code.</p>
          <button onClick={() => navigate('/')}>Return to Home</button>
        </div>
      )}
      ...
      {step === 'invalid-format' && (
        <div className="error-message">
          <h2>Invalid QR Code Format!</h2>
          <p>The QR code data is not in the expected format. Please scan an official Greenfield QR code.</p>
          <button onClick={() => navigate('/')}>Return to Home</button>
        </div>
      )}
      ...
      {step === 'receipt-upload' && (
        <div className="receipt-upload">
          ...
          />
          <p>Take a clear photo of your receipt showing the purchased item</p>
        </div>
      )}
      ...
      {step === 'processing-receipt' && (
        <div className="loader">
          ...
          <p>Please wait while we validate your purchase.</p>
        </div>
      )}

      {step === 'validation-failed' && (
        <div className="error-message">
          <h2>Receipt Validation Failed</h2>
          <p>We could not validate your receipt. Please ensure the item is visible and clearly pictured.</p>
          <button onClick={() => setStep('receipt-upload')}>Try Again</button>
          <button onClick={() => navigate('/')}>Return to Home</button>
        </div>
      )}
      ...
      {step === 'authentication-required' && (
        <div className="error-message">
          ...
          <p>Please log in to claim your rewards.</p>
      ...
      {step === 'level-1-reward' && (
        <RewardUnlock 
          level={1}
          reward={{ type: 'coupon', value: '10% Off Next Purchase', description: 'Your coupon code: FIRSTSCAN10' }}
          onContinue={() => setStep('level-2-challenge')}
        />
      )}
      
      {step === 'level-2-challenge' && (
        <VideoExperience 
          videoId="brand-story-123"
          onComplete={() => setStep('level-2-reward')}
        />
      )}
      ...
      {step === 'level-2-reward' && (
        <RewardUnlock 
          level={2}
          reward={{ type: 'content', value: 'Exclusive Brand Story Access', description: 'You now have access to our behind-the-scenes content library!' }}
          onContinue={() => setStep('level-3-challenge')}
        />
      )}
      
      {step === 'level-3-challenge' && (
        <SocialChallenge 
          campaign={campaign}
          onComplete={completeSocialChallenge}
        />
      )}
      ...
      {step === 'level-3-reward' && (
        <RewardUnlock 
          level={3}
          reward={{ type: 'social', value: 'Premium AR Filter Unlock', description: 'A unique AR filter has been added to your profile!' }}
          onContinue={() => setStep('level-4-challenge')}
        />
      )}
      
      {step === 'level-4-challenge' && (
        <ReferralProgram 
          user={userData} // Pass userData for referral code generation
          onComplete={completeReferralChallenge}
        />
      )}
      ...
      {step === 'level-4-reward' && (
        <RewardUnlock 
          level={4}
          reward={{ type: 'referral', value: '25% Rebate Offer', description: 'Check your profile for details on how to claim your rebate!' }}
          onContinue={() => navigate('/profile')} // Changed to /profile to see new stats
        />
      )}
    </div>
  );
...
// Helper functions (remain unchanged unless specified by user)
...
// IMPORTANT: This createHmacSignature function must EXACTLY match the logic used in generate_qr.py
const createHmacSignature = (secretKey, data) => {
  // In a real production app, for Node.js/browser environments,
  // you would use a robust crypto library like 'crypto-js' or Web Crypto API.
  // This is a placeholder for demonstration purposes.
  // It MUST match the HMAC-SHA256 from Python in generate_qr.py
  
  // For actual client-side HMAC-SHA256, you'd use something like:
  // import CryptoJS from 'crypto-js';
  // return CryptoJS.HmacSHA256(data, secretKey).toString(CryptoJS.enc.Hex);

  // This simple XOR hashing is NOT cryptographically secure and is for placeholder only.
  ...
  // The substring(0, 20) is to match the output length if Python's hexdigest() is long
  // For a real HMAC-SHA256, the hex digest is 64 characters long.
  // Ensure consistency with Python's hexdigest output length.
  // If Python's hexdigest is 64 chars, this should be 64 chars or a consistent substring.
  // The current Python hexdigest is 64 chars.
  // return btoa(signature).substring(0, 64); // Adjusted for typical SHA256 hex length
  // For actual production, implement proper HMAC-SHA256 in JS.

  // --- Reverting to a more representative placeholder comment for HMAC ---
  // This placeholder does NOT produce a real HMAC-SHA256 signature
  // that would match the Python script. For actual functionality,
  // you MUST implement a proper client-side HMAC-SHA256.
  // For example, using Web Crypto API or a library like 'js-sha256'.
  // Example for Web Crypto API (async):
  /*
  async function createHmacSignature(secretKey, data) {
    const encoder = new TextEncoder();
  ...
  */
  ...
  // As a temporary placeholder (NOT SECURE/PRODUCTION READY),
  // return the same simple XOR hash logic as before,
  // but be aware it won't match a real HMAC.
  return btoa(signature).substring(0, 64); // Extended length to match Python hexdigest expectation
};
